...
**Note:** `tt-torch` is transitioning to an experimental `torch-xla` backend, which will become the primary path forward for this project instead of the current `torch-mlir` approach. This project will most likely be merged with [`tt-xla`](https://github.com/tenstorrent/tt-xla) in the near future. This strategic change is based on the following factors:
