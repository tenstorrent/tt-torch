...
def shlo_to_flatbuffer(executor, module, compiler_config, len_activations, len_graph_constants):
    ...
    binary, ttnn = tt_mlir.compile_ttir_to_bytestream(ttir, executor.device, len_activations, len_graph_constants)
...
def _base_backend(gm, example_inputs, compiler_config, device):
    ...
    binary = shlo_to_flatbuffer(executor, shlo, compiler_config, len(example_inputs), len(graph_constants))
