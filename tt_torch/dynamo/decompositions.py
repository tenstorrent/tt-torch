...
def custom_pow(input, exponent):
    if isinstance(exponent, int):
        output = input
        for _ in range(exponent - 1):
            output = output * input
        return output
    return torch.exp(exponent * torch.log(input))
...
def scaled_dot_product_attention(
    ...
    attn_weight = torch._safe_softmax(attn_weight, dim=-1)
    attn_weight = torch.dropout(attn_weight, dropout_p, train=True)

    # TODO: I'm actually not sure what the correct secound output is supposed to be
    return attn_weight @ value, attn_mask
...
def _get_custom_decopositions() -> DecompositionTable:
    ...
    return {
        ...
        aten.pow.Tensor_Scalar: custom_pow,
