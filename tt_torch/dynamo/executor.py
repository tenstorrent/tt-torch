...
class Executor:
    ...
    def __call__(self, *inputs):
        ...
        """
        if self.compiler_config.compile_depth not in (CompileDepth.EXECUTE, CompileDepth.EXECUTE_CPP):
            assert (
        ...
        for device_idx, binary in self.mcg.binaries.items():
            ...
            if self.compiler_config.compile_depth == CompileDepth.EXECUTE_CPP:
                ...
                func_name = "main"
                is_successfull = tt_mlir.verify_cpp_ttnn(so_path, func_name, inputs, outputs, device)
                print("VERIFIATION OF CPP TTNN:", is_successfull)
