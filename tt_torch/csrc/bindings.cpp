...
void close_mesh_device(tt::runtime::Device &parent_mesh) {
  const char *system_desc_path = std::getenv("SYSTEM_DESC_PATH");
  if (system_desc_path) {
    std::remove(system_desc_path);
  }

  tt::runtime::closeMeshDevice(parent_mesh);
}
...
PYBIND11_MODULE(tt_mlir, m) {
  ...
  m.def("open_mesh_device", &open_mesh_device, py::arg("mesh_shape"),
        ...
        "system description");
  m.def("close_mesh_device", &close_mesh_device, py::arg("parent_mesh"),
        "Close the mesh device using new API and cleanup system description");
