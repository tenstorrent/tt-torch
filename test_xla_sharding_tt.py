...
# THIS FAILS DUE TO all_slice OP
# loc("p3.5"): error: 'sdy.all_slice' op operates on axis "_axis_0" which is already bound by a parent sdy.manual_computation op
  def test_xla_sharded_tensor_repr(self):
    ...
    # print("[HET DEBUG] MESH: ", mesh)
    partition_spec = (0, 1)
    xst = xs.mark_sharding(xt, mesh, partition_spec)
    self.assertTrue(isinstance(xst, XLAShardedTensor))

    xt_output = model(xt)
    hlo = torch_xla._XLAC._get_xla_tensors_hlo([xt_output])
    print("[HET DEBUG] HLO: ", hlo)
    self.assertTrue('XLAShardedTensor' not in str(xt_output)) # This line fails
...
# THIS PASSES
  def test_xla_shards(self):
    ...
    xt = xs.mark_sharding(t.to('xla'), mesh, (0,))
...
# THIS PASSES
  def test_replicated_xla_shards(self):
    ...
    mesh = self._get_mesh((self.n_devices,))
    t = torch.arange(num_element, dtype=torch.float32)
    xt = xs.mark_sharding(t.to('xla'), mesh, (None,))
    shards = xt.local_shards
    self.assertEqual(len(shards), self.n_devices)
    for i, shard in enumerate(shards):
      self.assertEqual(shard.data.device, torch.device('cpu'))
...
# THIS PASSES
  ...
  def test_partially_replicated_xla_shards(self):
    ...
    xt = xs.mark_sharding(t.to('xla'), mesh, (None, 1))
...
# THIS PASSES
  def test_load_local_shards(self):
    ...
    rt = xs.mark_sharding(t.to('xla'), mesh, (None,))
...
# THIS PASSES
  def test_xla_sharding_type(self):
    ...
    # Makes a 2,4 mesh on a t3k. Remember to update MESH_SHAPE env var
...
# THIS FAILS - EXPECTED (NEED TO AMEND TO SUPPORT V2 FORMAT)
...
# THIS FAILS DUE TO all_slice OP - (AMENDED TO SUPPORT V2 FORMAT)
  def test_mark_sharding_2d(self):
    ...
    if self.n_devices > 1:
      # Add an if statement to check if the "CONVERT_SHLO_TO_SHARDY" env var is set
      if os.environ.get("CONVERT_SHLO_TO_SHARDY", None) is None:
        # Using V1 sharding
        annotation = '{devices=[1,%d]%s}' % (self.n_devices, ','.join(
...
# THIS PASSES (need to look into why this passes but the one above doesn't)
  def test_mark_sharding_4d(self):
    ...
    xs.mark_sharding(xt, mesh, (0, 1, 2, 3))
    ...
    if self.n_devices > 1:
      if os.environ.get("CONVERT_SHLO_TO_SHARDY", None) is None:
        ...
        # Using V2 sharding
        annotation = '{devices=[1,1,%d,%d]<=[%d]}' % (
            z_dim, self.n_devices // z_dim, self.n_devices)
      self.assertEqual(annotation, torch_xla._XLAC._get_xla_sharding_spec(xt))
...
# THIS FAILS (on xm.wait_device_ops())
...
# THIS PASSES (amended to support V2 sharding)
  ...
  def test_tupled_partition_spec(self):
    ...
    actual = torch_xla._XLAC._get_xla_sharding_spec(t)
    if os.getenv("CONVERT_SHLO_TO_SHARDY", None) is None:
      # Using V1 sharding
      expected = "{devices=[%d]%s}" % (self.n_devices, ','.join(str(x) for x in range(self.n_devices)))
    else:
      # Using V2 sharding
      expected = "{devices=[%d]<=[%d]}" % (self.n_devices, self.n_devices)
    self.assertEqual(actual, expected)

# THIS FAILS - Expected since the V2 implementation doesn't handle tupled partition specs
# well yet
  @unittest.skipUnless(xr.global_runtime_device_count() >= 4,
                       "Multiple devices required for tupled partition spec")
  def test_named_partial_tupled_partition_spec(self):
    ...
    os.environ["MESH_SHAPE"] = f"1, 2, {self.n_devices // 2}"
...
# THIS PASSES - amended to support V2 sharding
  ...
  def test_multiple_tuples_in_spec(self):
    mesh = xs.Mesh(
        ...
        ('a', 'b', 'c', 'd'))
    os.environ["MESH_SHAPE"] = f"1, 2, {self.n_devices // 2}, 1"
    t = torch.randn(2, 2).to('xla')
    xs.mark_sharding(t, mesh, (('a', 'b'), ('c', 'd')))
    actual = torch_xla._XLAC._get_xla_sharding_spec(t)
    if os.getenv("CONVERT_SHLO_TO_SHARDY", None) is None:
      # V1 sharding
      expected = "{devices=[2,%d]%s}" % (self.n_devices // 2, ','.join(str(x) for x in range(self.n_devices)))
    else:
      # V2 sharding
      expected = "{devices=[2,%d]<=[%d]}" % (self.n_devices // 2, self.n_devices)
    self.assertEqual(actual, expected)

# THIS FAILS due to V2 sharding not supported
# V1 sharding output: {devices=[1,2,4]0,1,2,3,4,5,6,7}
# V2 sharding output: {devices=[1,2,4]<=[8] last_tile_dim_replicate}
# Need to confirm whether the `last_tile_dim_replicate` is expected in V2
  @unittest.skipUnless(xr.global_runtime_device_count() > 1,
...
# THIS FAILS due to xm.wait_device_ops()
...
# THIS PASSES
  def test_clear_sharding(self):
    xt = torch.randn(2, 4, 8, 16).to('xla')
...
# THIS FAILS
# error: Could not apply propagated tensor shardings to tensor dimensions
# error: Could not update manual axes for manual computation op
# loc("broadcast.5"): error: Could not apply propagated tensor shardings to tensor dimensions
...
# THIS FAILS
# loc("p4.7"): error: 'sdy.all_slice' op operates on axis "_axis_0" which is already bound by a parent sdy.manual_computation op
...
# error: Could not update shapes based on their tensor sharding attributes
  ...
  def test_shard_hashing(self):
    xt1 = torch.ones(2, 2).to('xla')
    xt2 = torch.ones(2, 2).to('xla')
...
# error: Could not apply propagated tensor shardings to tensor dimensions
# error: Could not update manual axes for manual computation op
# loc("add.6"): error: Could not apply propagated tensor shardings to tensor dimensions
# error: Could not update shapes based on their tensor sharding attributes
...
# THIS FAILS for num_devices = 8
# RuntimeError: too many indices for tensor of dimension 2
...
# THIS FAILS
# loc("p1.5"): error: 'sdy.all_slice' op operates on axis "_axis_0" which is already bound by a parent sdy.manual_computation op
...
# THIS FAILS
# loc("p0.9"): error: 'sdy.all_slice' op operates on axis "_axis_0" which is already bound by a parent sdy.manual_computation op
  # @unittest.skipUnless(xr.global_runtime_device_count() > 1,
...
# THIS FAILS
# loc("p1.3"): error: 'sdy.all_slice' op operates on axis "_axis_0" which is already bound by a parent sdy.manual_computation op
  # @unittest.skipUnless(xr.global_runtime_device_count() > 1,
  #                      "Multiple devices required for autograd sharding test")
...
# THIS PASSES
  def test_named_partition_spec(self):
    xt1 = torch.arange(64).reshape(8, 8).to('xla')
    mesh = xs.Mesh(
...
# THIS FAILS
# loc("rng-bit-generator.23"): error: failed to legalize operation 'stablehlo.rng_bit_generator'
...
# THIS FAILS - need to figure out how to register a counter for CreateIotaOpSharding
...
# THIS FAILS (amended for V2 shardings)
# error: Could not apply propagated tensor shardings to tensor dimensions
# error: Could not update manual axes for manual computation op
...
# THIS FAILS - during tiled sharding
# error: Mesh is not valid
  def test_from_cpu_shards_2d(self):
    ...
    print("HET DEBUG MESH SHAPE: ", mesh_shape)
    ...
    if os.getenv("CONVERT_SHLO_TO_SHARDY", None) is None:
      # V1 shardings
      op_sharding_func = mesh_2d.get_op_sharding
    else:
      # V2 shardings
      op_sharding_func = mesh_2d.get_op_sharding_v2
    op_sharding = op_sharding_func(partition_spec)
    print("HET DEBUG OPSHARDING: ", op_sharding)
    global_tensor = from_cpu_shards(shards, op_sharding)
    print("HET DEBUG GLOBAL TENSOR: ", global_tensor)
    self.assertTrue(torch.allclose(global_tensor.cpu(), shards[0]))

    if self.n_devices > 1:
      # Tiled sharding
      shards = [torch.LongTensor([[i]]) for i in range(self.n_devices)]
      print("HET DEBUG SHARDS: ", shards)
      partition_spec = (0, 1)
      op_sharding = op_sharding_func(partition_spec)
      global_tensor = from_cpu_shards(shards, op_sharding)
      print("HET DEBUG GLOBAL TENSOR TILED: ", global_tensor)
...
# THIS FAILS
# loc("custom-call.4"): error: failed to legalize operation 'stablehlo.custom_call'
  def test_manual_sharding_e2e(self):
    ...
    xxx = torch_xla._XLAC._spmd_shard_to_full_shape(
        xxt.global_tensor, op_sharding, x.shape,
        x.dtype)
...
# THIS FAILS
# somehow uses V1 shardings. Need to look closer into implementing V2 support for
# manual sharding.
...
# THIS FAILS
# V1 sharding: {devices=[8,1,1,1]0,1,2,3,4,5,6,7}
# V2 sharding: {devices=[8,1,1]<=[8]}
# Check with Sungjoon to see if the V2 sharding is correct
  @unittest.skipUnless(
  ...
  def test_data_loader_with_sharding(self):
    device = torch_xla.device()
    mesh = xs.get_1d_mesh("data")
    os.environ["MESH_SHAPE"] = f"{','.join(str(i) for i in mesh.mesh_shape)}"
    print("MESH: ", mesh)
    print("MESH_SHAPE env: ", os.environ["MESH_SHAPE"])
    batch_size = 8
...
# THIS FAILS
# Always |    FATAL | Cannot create borrowed tensor with null data
  @unittest.skipUnless(
      xr.global_runtime_device_count() > 1,
      "Multiple devices required for dataloader sharding test")
...
#   File "/localdev/hshah/tt-torch/test_xla_sharding_tt.py", line 1651, in test_fallback
...
# THIS PASSES
  def test_valid_mesh_creation(self):
    ...
    os.environ["MESH_SHAPE"] = f"{','.join(str(i) for i in mesh_shape)}"
...
# THIS FAILS
# Somehow still uses V1 sharding
  @unittest.skipIf(xr.global_runtime_device_count() == 1,
                   "Multiple devices needed")
