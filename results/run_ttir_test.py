...
def process_excel_sheet(excel_path, runner_idx, runner_total):
    """
    Parse models excel sheet to extract the raw TTIR and dump in a file.
    Compile TTIR graph to TTNN graph and then translate it to flatbuffer binary file.
    Execute the binary file with TTRT and record the stack dump in case of failure.
    """
...
def update_model_file(dir, excel_path):
    """
    Update 1) the status in case of successful execution or 2) add the error and
    stack dump in case of failure in original models excel file.
    """
