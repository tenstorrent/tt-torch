...
def apply_tensor_parallel_sharding_base(
    ...
    for layer in model.layers:
        ...
        for expert in layer.block_sparse_moe.experts:
            # Original
            xs.mark_sharding(expert.w1.weight, mesh, ("batch", "model"))
            xs.mark_sharding(expert.w2.weight, mesh, ("model", "batch"))
            xs.mark_sharding(expert.w3.weight, mesh, ("batch", "model"))

            # Flipped
            # xs.mark_sharding(expert.w1.weight, mesh, ("model", "batch"))
            # xs.mark_sharding(expert.w2.weight, mesh, ("batch", "model"))
            # xs.mark_sharding(expert.w3.weight, mesh, ("model", "batch"))
