...
def test_all_reduce(shard_dim):
    ...
    mesh = create_device_mesh((2, xr.global_runtime_device_count() // 2), ("batch", "model"))
...
def test_all_gather(shard_dim):
    ...
    setup_xla_environment_for_tp()
    num_rows = 2
    num_cols = xr.global_runtime_device_count() // num_rows
    mesh = create_device_mesh((num_rows, num_cols), ("batch", "model"))
