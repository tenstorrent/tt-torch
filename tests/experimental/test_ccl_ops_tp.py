...
def test_all_gather(shard_dim):
    ...
    y = y.to("cpu") # y has shape [16384, 784]

    chunks = torch.chunk(y, len(groups[0]), dim=shard_dim)
    print(f"Shard dim: {shard_dim}, Chunk Shape: {chunks[0].shape}, Golden Shape: {golden.shape}")

    assert all(torch.equal(chunks[0], chunk) for chunk in chunks[1:]), "All chunks should be exactly equal"
