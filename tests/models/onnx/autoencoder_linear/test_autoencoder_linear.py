...
import pytest
from tests.utils import ModelTester, OnnxModelTester
...
class ThisTester(OnnxModelTester):
    def _load_model(self):
        torch_model = LinearAE()
        torch_input = self._load_inputs()
        torch.onnx.export(torch_model, torch_input, "LinearAE.onnx", opset_version=9)
        return onnx.load("LinearAE.onnx")
