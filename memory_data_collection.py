...
def run_test(test_name, iterations, output_dir, username):
    ...
    with open(csv_path, 'w', newline='') as csvfile:
        ...
        writer.writerow(['iteration', 'start_memory_mb', 'peak_memory_mb', 'end_memory_mb', 'execution_time_s', 'status'])
    ...
    for i in range(1, iterations + 1):
        ...
        with open(csv_path, 'a', newline='') as csvfile:
            writer = csv.writer(csvfile)
            writer.writerow([i, round(start_memory, 2), round(peak_memory, 2), 
                            round(end_memory, 2), round(execution_time, 2), status])
        
        # Clean cache
        print(f"\nCleaning cache: rm -rf /localdev/{username}/cache/*")
        try:
            subprocess.run(f"rm -rf /localdev/{username}/cache/*", shell=True, check=True)
        except subprocess.CalledProcessError as e:
            print(f"Warning: Failed to clean cache: {e}")
        
        print(f"Iteration {i} completed in {execution_time:.2f}s")
        print(f"Memory usage: Start: {start_memory:.2f}MB, Peak: {peak_memory:.2f}MB, End: {end_memory:.2f}MB")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Run pytests multiple times and track memory usage')
    parser.add_argument('--tests', nargs='+', default=['clip', 'flan_t5', 'vilt', 'codegen', 'RMBG', 'mgp-str-base'], 
                        help='List of test names to run')
    parser.add_argument('--iterations', type=int, default=5, 
             
                        help='Number of iterations to run each test')
    parser.add_argument('--output-dir', type=str, default='./memory_results', 
